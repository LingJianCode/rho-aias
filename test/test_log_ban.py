#!/usr/bin/env python3
"""
WAF / Rate Limit 日志触发封禁集成测试

测试流程:
1. 启动 rho-aias，配置监听临时日志文件（WAF、rate_limit）
2. 向日志文件追加真实格式的日志行
...
class LogBanTestBase(unittest.TestCase):
    ...
    def setUp(self):
        # 创建日志目录并清空日志文件
        os.makedirs(TEST_LOG_DIR, exist_ok=True)
        for path in [WAF_LOG_PATH, RATE_LIMIT_LOG_PATH]:
...
class TestMixedBan(LogBanTestBase):
    ...
    def test_same_ip_different_sources(self):
        ...
        banned = wait_for_ban(self.api_client, "203.0.113.99", source="waf", timeout=20)
        self.assertTrue(banned, "IP 203.0.113.99 should be banned by waf")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="WAF / Rate Limit Log Ban Integration Tests",
...
测试运行示例:
  ...
  # 运行特定测试
  python3 %(prog)s --test TestRateLimitBan.test_rate_limit_ban
        """
    )
    parser.add_argument(
        "-t", "--test",
        help="Run specific test (e.g., TestWAFBan.test_waf_interrupted)"
    )
    parser.add_argument(
        "-v", "--verbose",
        action="store_true",
