 ...
 *
 * 黑名单模式不做此优化:
 * - 黑名单模式是封禁特定国家，必须对所有包做检查，否则攻击者可通过
 *   设置 ACK 位或发送非 SYN 包来绕过封禁
 *
...
static __always_inline int check_geo_blocking(struct packet_info *pi) {
    ...
    if (pi->eth_proto == ETH_P_IP) {
        // TCP 连接状态感知（仅白名单模式）:
        // 白名单模式下，非纯 SYN 包放行（已建立连接的响应包）
        // 黑名单模式必须对所有包做检查，防止攻击者通过设置 ACK 位绕过
        if (config->mode == 0 && pi->ip_protocol == IPPROTO_TCP) {
