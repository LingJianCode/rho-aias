...
type BanRecord struct {
	...
	IP        string    `gorm:"size:45;not null;index" json:"ip"`                  // 封禁 IP
	Source    string    `gorm:"size:20;not null;index" json:"source"`               // 封禁来源: waf, rate_limit, anomaly
	Reason    string    `gorm:"size:255" json:"reason"`                             // 封禁原因
	Duration  int       `gorm:"not null" json:"duration"`                           // 封禁时长（秒）
	Status    string    `gorm:"size:20;not null;default:active;index" json:"status"` // 状态: active, expired, manual_unblock
