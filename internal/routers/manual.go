...
// RegisterBlacklistRoutes 注册手动规则管理路由（黑名单）
// 路由路径: /api/manual/blacklist/rules
func RegisterBlacklistRoutes(group *gin.RouterGroup, manualHandle *handles.BlacklistHandle, enforcer *casbin.Enforcer, authService *services.AuthService, apiKeyService *services.APIKeyService) {
	...
	blacklist.GET("/rules",
		...
		manualHandle.ListBlacklistRules,
	)
}
