...
import { getWhitelist, addWhitelistRule, addWhitelistRulesBatch, deleteWhitelistRule, deleteWhitelistRules } from '@/api/manual'
...
async function fetchRules() {
  loading.value = true
  try {
    const res = await getWhitelist({ page: page.value, page_size: pageSize.value })
...
async function handleBatchAdd() {
  ...
  try {
    const res = await addWhitelistRulesBatch(batchForm.ips, batchForm.reason)
...
async function handleDelete(row: ManualRuleItem) {
  ...
  try {
    await deleteWhitelistRule(row.id)
...
async function handleBatchDelete() {
  ...
  try {
    await deleteWhitelistRules(selectedIds.value)
    ElMessage.success('删除成功')
    fetchRules()
  } catch {
    // Error handled
